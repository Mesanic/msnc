...
Repo has `.scope/` → run `impact <file|symbol>` before each edit, as `node "${CLAUDE_PLUGIN_ROOT}/skills/scope/scripts/scope.mjs" impact …`; the covering tests it names are the affected tests. No `.scope/` → grep every caller.
