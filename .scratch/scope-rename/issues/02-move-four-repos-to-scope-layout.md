...
Per repo, in order:
1. **Ticket-04 commit:** commit rollout ticket 04's uncommitted changes (vendored scope/atlas/scalpel folders removed, Scope hooks gone), and in the same commit remove the `<!-- atlas:begin -->` block from `CLAUDE.md` in Maji, Roundwell and Roundwell v2.
...
Then close rollout ticket 04 (its progress commit is on MSNC's local branch `chore/rollout-04-done`): Map v2 is dropped because it is archived (`Mesanic/map-v2`); mark both boxes done. Tell the user the two backup folders (`~/.claude/backups/scope-migration-2026-09-23/`, `~/.claude/backups/tool-comparison-engines-2026-09-24/`) can be deleted; the user deletes them.
...
- The backups `~/.claude/backups/scope-migration-2026-09-23/` and `~/.claude/backups/tool-comparison-engines-2026-09-24/` can now be deleted by the user.
