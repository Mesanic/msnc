...
- 2026-09-23 · Box 3 closed. A throwaway probe case (deleted, never committed) on the Scope fixture ran exactly `sed -i 's/email/email/' src/users.js` through Bash. In 3/3 runs the gate refused it: "no impact check has run (via Bash)", listed in `permission_denials`. So Claude Code 2.1.281 honours the `if: "Bash(sed *)"` filter. `cp` and `rm` weren't probed. The same rerun, after ticket 22 put Node in the sandbox: pace 3/3; implement-commits 2/3 (ticket 27); failing-check 1/3 (ticket 26); impact 0/3 (ticket 25: the gate can't see an impact log written in sandboxed Bash); gate case 2/3 (ticket 24). All three boxes met. Details in `evals/RESULTS.md`, "Rerun after Node in the sandbox (ticket 22)".
