...
- Install Node at a system path the sandbox can read, for example `/usr/bin/node`. Recommended: Node 22 (the version the hooks already run on here, v22.22.2), for example from NodeSource. Ubuntu 24.04's own `nodejs` package is 18.19.1: it has `node --test`, but `sextant.mjs` hasn't been tried on it.
...
- Evidence (kept sandboxes, `out/trace.jsonl`):
  ...
  - `indexed-repo-runs-impact-before-edit` (`-mtJ4Cl`, `-zRtuaW`, `-ViETGz`): `node …/sextant.mjs impact createUser` → `node: command not found`, so the impact log stayed empty, the Scope gate refused the Edit ("src/users.js is in the code map and no impact check has run"), and `lower-cased` failed in all 3 runs.
