...
Doctor only reads. It never edits a file or a setting; it reports and names the fix. `/msnc:declutter` applies fixes one item at a time.
...
```bash
node "${CLAUDE_SKILL_DIR}/doctor.mjs"
```
...
MSNC:
[doctor.mjs lines, each flagged one with its fix]
