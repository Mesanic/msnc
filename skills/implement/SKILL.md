...
- Scope index: the repo uses Scope when `.atlas/` exists or `.gitignore` lists `.atlas/…` entries. `.gitignore` lists `.atlas/…` entries but `.atlas/` is missing → run `/msnc:scope init`, then commit its output alone as `chore: build Scope index` (`git add .atlas .map .gitignore`), so the tree is clean before the first ticket. Neither → leave Scope alone: no init, no index.
...
7. Commit this ticket alone: stage the files the subagent reported plus `docs/decisions.md`, and with a Scope index stage the rescan's `.atlas/` and `.map/` changes too (`git add .atlas .map` takes only what `.gitignore` leaves tracked), so the tree is clean after the commit (other stray changes in `git status` → ask), message `<feat|fix|refactor>: <ticket title>`, then a body line `Why: <the ticket's Why line>`, plus `Closes #N` for GitHub tickets. Local tickets: set `**Status:** done`, tick the acceptance boxes and add the comments in the same commit. Never close or edit a parent issue.
