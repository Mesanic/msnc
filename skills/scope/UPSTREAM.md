...
## This copy = pin + two patches

Upstream at the pin plus `patches/sextant/0001-*.patch` and `0002-*.patch` (repo root), not yet pushed to `Mesanic/sextant` (rollout ticket 01). Until they land and the pin moves, `sync-upstream --check` reports `scripts/impact-fallback.test.mjs` as local only, and part of the `scripts/sextant.mjs` change comes from them.
