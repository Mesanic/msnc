...
```
/msnc:aside <your question>
/msnc:aside what does this function actually return?
/msnc:aside is this pattern thread-safe?
/msnc:aside why are we using X instead of Y here?
/msnc:aside what's the difference between foo() and bar()?
/msnc:aside should we be worried about the N+1 query we just added?
```
...
- What step was in progress at the moment `/msnc:aside` was invoked?
...
**No question provided (`/msnc:aside` with nothing after it):**
Respond:
```
...
If the question implies changing what is being built (e.g., `/msnc:aside actually, let's use Redis instead`), clarify:
```
...
**No active task (nothing in progress when `/msnc:aside` is invoked):**
...
**Multiple `/msnc:aside` questions in a row:**
...
```
User: /msnc:aside what does fetchWithRetry() actually do?
...
```
User: /msnc:aside is the approach we're taking thread-safe?
