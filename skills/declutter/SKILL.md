---
name: declutter
description: Garbage collection for your Claude Code configuration. Scans ~/.claude (skills, memory, hooks, permissions, MCP servers, caches) and the overlaps /msnc:doctor finds for redundant, stale, orphaned or low-value items, moves each confirmed item to a trash folder, and asks per item.
disable-model-invocation: true
metadata:
  origin: ECC
---

# Declutter — Garbage Collection for Claude Code Setups
...
- After installing a large skill pack (e.g. MSNC itself), to reconcile overlaps with existing setup
...
| 9 | MSNC overlaps | `/msnc:doctor` output | `Duplicate skills:` lines (personal or project skills MSNC now ships; plugins that duplicate it); `Project settings:` lines (project `enabledPlugins` entries that re-enable a duplicating plugin, or overrides that no longer matter) |

## Workflow

1. **Scan** all channels (or the subset the user names). For channel 9, run `/msnc:doctor` first and take its flagged lines as candidates. Collect candidates with: path, channel, signal that flagged it, size, last-modified.
...
4. **Soft-delete confirmed items**: prefer `.disabled` rename for skills/hooks and `_gc_trash/<date>/` move for files. Permission and `enabledPlugins` entries live in JSON (no comments possible): back up the settings file, record each removed entry verbatim in `gc_log.md`, then remove it from the `allow` array or `enabledPlugins` object with `jq`. Only hard-delete when the user explicitly asks.
...
- **Touching anything outside `~/.claude`** (or the project's `.claude/`). Declutter never wanders into source trees.
...
- `/msnc:doctor` — finds duplicates, conflicting project settings and old Scope layouts, and never changes anything. Declutter is the half that removes, one confirmed item at a time.
